...
import {TransparentUpgradeableProxy__factory, PrimitiveContract__factory, ProxyAdmin__factory} from "../typechain-types";
...
async function main() {
    ...
    if(true){
        ...
        const contractAddress = "0x0b02E1dFdF9c222B8F4Ee207BD656D5612f03326";
        const primitiveContract = PrimitiveContract__factory.connect(contractAddress, hre.ethers.provider);
...
async function deployPrimitiveContract(deployer: any, verifier: string) {
    if(false){
        //deploy
        const implementation = await hre.ethers.deployContract("PrimitiveContract", {
            signer: deployer
        });
        await implementation.waitForDeployment();
        console.log(`[impl] PrimitiveContract deployed to ${implementation.target.toString()}`);
        const calldata = PrimitiveContract__factory.createInterface().encodeFunctionData("initialize", [verifier, ruleID]);
